...
// saveString(exporter.parse( out ), "out.obj");
// saveString(exporter.parse( lun ), "lun.obj");
// saveString(exporter.parse( gun ), "gun.obj");
// saveString(exporter.parse( lunS ), "lunS.obj");
...
function createOut(r:number): THREE.Object3D<THREE.Event> {
  ...
  for (let index = 0; index < 360/div; index++) {
    ...
    path.push(...curPs);
    curPs.forEach(item=>{
      path.push(item.clone().add(offset));
    })
